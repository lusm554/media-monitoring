...
class Scraper:
  def __init__(self, from_rss, from_google, from_dzen, article_wrp):  
    self.from_rss = from_rss()
    self.from_google = from_google()
    self.from_dzen = from_dzen()
  ...
  def get_articles_from_dzen(self):
    dzen_articles = self.from_dzen.fetch_articles()
    return dzen_articles
  ...
  def get_articles(self, time_period='24h'):
    assert time_period in ('24h', 'all_available'), f'Parameter time_period {time_period!r} not found, try 24h'
    union_articles = [
      *self.get_articles_from_rss(),
      *self.get_articles_from_google(time_period),
      *self.get_articles_from_dzen(),
    ]
...
def get_scraper_instance(rss_scrp, dzen_scrp, go_scrp, article_wrp):
  scraper = Scraper(from_rss=rss_scrp, from_dzen=dzen_scrp, from_google=go_scrp, article_wrp=article_wrp)
  return scraper
